...
app.use((req, res, next) => {
  res.header("Access-Control-Allow-Origin", "http://www.flai.ml");
...
app.get('/', (req, res) => {
	res.send("<h1><a href='http://www.flai.ml' >Go to flai</a></h1>")
})
...
app.post('/download', (req, res) => {
	...
	if (password === process.env.PASS) {
		...
		return res.redirect('http://www.flai.ml/#/error');
...
app.get('/link/:id', (req, res) => {
	...
	if(fetchedLink.length < 10) res.redirect('http://www.flai.ml');
			...
			if(url && extension) {
				if(url[4] !== 's') {
					try {
						...
						res.redirect('http://www.flai.ml/#/error');
					}
				}
				else {
					try {
